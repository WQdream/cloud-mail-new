...
const accountService = {
	...
	async setRemark(c, params, userId) {
		const { remark, accountId } = params
		if (remark && remark.length > 100) {
			throw new BizError(t('remarkLengthLimit'));
		}
		await orm(c).update(account).set({remark: remark || ''}).where(and(eq(account.userId, userId),eq(account.accountId, accountId))).run();
	}
};
