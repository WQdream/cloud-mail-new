<template>
  <div class="account-box">
              ...
              <el-dropdown v-else>
                ...
                <template #dropdown>
                  <el-dropdown-menu>
                    ...
                    <el-dropdown-item v-if="hasPerm('email:send')" @click="openSetRemark(item)">{{ $t('setRemark') }}
                    </el-dropdown-item>
    ...
    <el-dialog v-model="setRemarkShow" :title="$t('setRemark')">
      <div class="container">
        <el-input v-model="accountRemark" type="textarea" :rows="3" :placeholder="$t('remark')" autocomplete="off">
        </el-input>
        <el-button class="btn" type="primary" @click="setRemark" :loading="setRemarkLoading"
        >{{ $t('save') }}
        </el-button>
      </div>
    </el-dialog>
...
import {accountList, accountAdd, accountDelete, accountSetName, accountSetRemark} from "@/request/account.js";
...
const setRemarkLoading = ref(false)
const accountRemark = ref(null)
const addRef = ref({})
...
function openSetRemark(accountItem) {
  accountRemark.value = accountItem.remark || ''
  account = accountItem
  setRemarkShow.value = true
}

function setRemark() {
  let remark = accountRemark.value
  ...
  setRemarkLoading.value = true
  accountSetRemark(account.accountId, remark).then(() => {
